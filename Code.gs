...
function handleGetAdminData(adminUser) {
  ...
  if (vSheet && vSheet.getLastRow() > 1) {
    ...
    for (var j = 1; j < vData.length; j++) {
      violations.push({ timestamp:vData[j][0], examId:vData[j][1], examName:vData[j][2], studentName:vData[j][3], deviceId:vData[j][4], count:parseInt(vData[j][5])||0, userAgent:vData[j][6] });
    }
  }
...
function handleReportViolation(payload) {
  try {
    ...
    var vSheet = ss.getSheetByName(VIOLATIONS_SHEET_NAME);
    if (vSheet) {
      // Kolom: timestamp, examId, examName, studentName, deviceId, violationCount, userAgent
...
function handleGetViolations(adminUser) {
  ...
  if (vSheet && vSheet.getLastRow() > 1) {
    ...
    for (var i = data.length-1; i >= 1; i--) {
      violations.push({ timestamp:data[i][0], examId:data[i][1], examName:data[i][2], studentName:data[i][3], deviceId:data[i][4], count:parseInt(data[i][5])||0, userAgent:data[i][6] });
    }
  }
